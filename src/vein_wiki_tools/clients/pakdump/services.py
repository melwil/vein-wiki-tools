...
from vein_wiki_tools.clients.pakdump.models import UEBlueprintGeneratedClass, UEItemType, UEModel, UEReference
...
async def get_construction_info(node: Node, graph: Graph) -> Construction | None:
    ...
    if maintenance_costs := node.ue_model.get_prop("maintenance_cost"):
        for cost in maintenance_costs:
            ...
            result.build_requirements.append(ItemCountReference(text=cost_model.display_name(), count=cost.quantity))
